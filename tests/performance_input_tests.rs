...
mod performance_input_integration_tests {
    ...
    fn test_various_event_types_performance_classification() {
        ...
        assert_eq!(
            Event::TrackAndField(TrackAndFieldEvent::M200).performance_type(),
        ...
        assert_eq!(
            Event::TrackAndField(TrackAndFieldEvent::M3000mSC).performance_type(),
            PerformanceType::Time
        );
        assert_eq!(
            Event::TrackAndField(TrackAndFieldEvent::M4x100m).performance_type(),
            PerformanceType::Time
        );

        // Verify road running events are time-based
        assert_eq!(
            Event::RoadRunning(RoadRunningEvent::Road10km).performance_type(),
            PerformanceType::Time
        );
        assert_eq!(
            Event::RoadRunning(RoadRunningEvent::RoadHM).performance_type(),
            PerformanceType::Time
        );

        // Verify race walking events are time-based
        assert_eq!(
            Event::RaceWalking(RaceWalkingEvent::Road20kmW).performance_type(),
            PerformanceType::Time
        );
        assert_eq!(
            Event::RaceWalking(RaceWalkingEvent::M15000mW).performance_type(),
            PerformanceType::Time
        );
    ...
    fn test_performance_parsing_error_handling() {
        ...
        let error_msg = result.unwrap_err();
        assert!(
            error_msg.contains("Invalid hours")
                || error_msg.contains("Invalid minutes")
                || error_msg.contains("Invalid seconds")
        );
    ...
    fn test_distance_parsing_validation() {
        ...
        for distance_str in invalid_distances {
            ...
            assert!(
                result.is_err(),
                "Should fail to parse invalid distance: {}",
                distance_str
            );
        }
    }
